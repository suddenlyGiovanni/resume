...
describe('date utils tests', () => {
	it('timezone should return UTC', ({ expect }) => {
	...
	it('unixTime should have correct ISO string', ({ expect }) => {
		expect(unixTime.toISOString()).toBe('1970-01-01T00:00:00.000Z')
	})

	it('unixTime should have correct timestamp', ({ expect }) => {
		expect(unixTime.getTime()).toBe(0)
	})

	it('unixTime should have correct UTC string', ({ expect }) => {
		expect(unixTime.toUTCString()).toBe('Thu, 01 Jan 1970 00:00:00 GMT')
	})

	it('dayBeforeUnixTime should have correct ISO string', ({ expect }) => {
